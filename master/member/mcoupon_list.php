...
$where = " and section = '".$_SESSION['admin_yonex_section']."' ";

if($s_sect1){
	$where .= " and member_sect = '".$s_sect1."' ";
}

if($s_sect2){
	$where .= " and member_level = '".$s_sect2."' ";
}
...
$query = "select * from member_coupon_set where 1=1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
//echo "<br><br>쿼리 = ".$query."<br><Br>";

$result = mysqli_query($gconnet,$query);

$query_cnt = "select idx from member_coupon_set where 1=1 ".$where;
...
<!--
	function go_view(no){
		location.href = "mcoupon_view.php?idx="+no+"&pageNo=<?=$pageNo?>&bmenu=<?=$bmenu?>&smenu=<?=$smenu?>&field=<?=$field?>&keyword=<?=urlencode($keyword)?>";
	}
	...
	function go_regist(){
		location.href = "mcoupon_write.php?bmenu=<?=$bmenu?>&smenu=<?=$smenu?>&v_sect=<?=$v_sect?>&s_professor=<?=$s_professor?>";
	}
						...
						<tr>
							...
							<td colspan="5">
								<select name="field" size="1" style="vertical-align:middle;width:20%;">
									...
									<option value="coupon_price" <?=$field=="coupon_price"?"selected":""?>>쿠폰 액면가격</option>
								</select>
			...
			<!-- 엑셀 출력을 위한 전송 폼 종료 -->
				...
				<thead>
					<tr>
						<th width="5%">번호</th>
						<!--<th width="10%">쿠폰종류</th>-->
						<th width="35%">쿠폰 간략설명</th>
						<th width="10%">할인종류</th>
						<th width="10%">할인혜택</th>
						<th width="20%">쿠폰만료일</th>
						<th width="20%">생성일</th>
					</tr>
				</thead>
				<tbody>
