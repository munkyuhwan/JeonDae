	...
	//echo "시작페이지 = ".$start_page." | 현재페이지 = ".$c_page." || current_page = ".$current_page." ||| 종료페이지 = ".$end_page." |||| 총 페이지수 = ".$total_page."<br><br>";
			...
			###처음 버튼
				if ($c_page > $page_size) {
					...
					echo '<a href="'.$_SERVER["PHP_SELF"].'?pageNo_sub='.$prev_list.'&'.$total_param_sub.'" class="prev">이전</a>';
				}
				//echo '&nbsp;';
			###페이지 출력
				for ($i=$start_page;$i <= $end_page;$i++){
					...
					if($c_page != $page){
						...
						echo '<a href="'.$_SERVER["PHP_SELF"].'?pageNo_sub='.$page.'&'.$total_param_sub.'">'.$i.'</a> ';
					}else{
						//echo "<span class='page'>".($i+1).'</span>';
						echo '<a href="'.$_SERVER["PHP_SELF"].'?pageNo_sub='.$c_page.'&'.$total_param_sub.'" class="on">'.$i.'</a> ';
					}
					if($i == $end_page){
						//echo '<span>|</span>';
					}
...
;					}
			...
			###다음버튼
				if($total_page > $end_page){
					...
					echo '<a href="'.$_SERVER["PHP_SELF"].'?pageNo_sub='.$next_list.'&'.$total_param_sub.'" class="next">다음</a>';
					// 처음
