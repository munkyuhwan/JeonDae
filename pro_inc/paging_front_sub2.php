...
if($num_sub2 >= 1){ // DB 추출갯수가 하나라도 있을때 시작 
	...
	//echo "시작페이지 = ".$start_page." | 현재페이지 = ".$c_page." || current_page = ".$current_page." ||| 종료페이지 = ".$end_page." |||| 총 페이지수 = ".$total_page."<br><br>";
			...
			###처음 버튼
				if ($c_page > $page_size) {
					//마지막
					//echo '<a href="'.$_SERVER["PHP_SELF"].'?pageNo_sub=&'.$total_param_sub.'&pidx='.$pidx.'"><img src="'.$last_img_path.'" border="0"  alt="처음으로"/></a>';
					//이전
					...
					echo ' <a href="'.$_SERVER["PHP_SELF"].'?pageNo_sub2='.$prev_list.'&'.$total_param_sub2.'" class="btn_in"><img src="'.$prev_img_path.'" align="absmiddle"  alt="이전 10개" /></a>';
				}
				//echo '&nbsp;';
			###페이지 출력
				for ($i=$start_page;$i <= $end_page;$i++){
					...
					if($c_page != $page){
						...
						echo '<span>|</span> <a href="'.$_SERVER["PHP_SELF"].'?pageNo_sub2='.$page.'&'.$total_param_sub2.'">'.$i.'</a> ';
					}else{
						//echo "<span class='page'>".($i+1).'</span>';
						echo '<span>|</span> <a href="'.$_SERVER["PHP_SELF"].'?pageNo_sub2='.$c_page.'&'.$total_param_sub2.'" class="on">'.$i.'</a> ';
					}
...
;					}
			...
			###다음버튼
				if($total_page > $end_page){
					...
					echo '<span>|</span><a href="'.$_SERVER["PHP_SELF"].'?pageNo_sub2='.$next_list.'&'.$total_param_sub2.'" class="btn_in"><img src="'.$next_img_path.'" align="absmiddle"  alt="다음 10개"/></a>';
					// 처음
					...
					//echo ' <a href="'.$_SERVER["PHP_SELF"].'?pageNo_sub='.$last_page.'&'.$total_param_sub.'&pidx='.$pidx.'"> <img src="'.$first_img_path.'" border="0"   alt="마지막으로" /></a>';
				}
