...
if($num >= 1){ // DB 추출갯수가 하나라도 있을때 시작 
	...
	$page_size = 20; // 1 ~ 10 페이지까지 화면에 뿌려준다.
	...
	//echo "시작페이지 = ".$start_page." | 현재페이지 = ".$c_page." || current_page = ".$current_page." ||| 종료페이지 = ".$end_page." |||| 총 페이지수 = ".$total_page."<br><br>";
			...
			###처음 버튼
				...
				<?
					...
					?>
						<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$prev_list?>');" class="prev">PREV</a>
					<?
				}
			...
			###페이지 출력
				for ($i=$start_page;$i <= $end_page;$i++){
					...
					?>
						<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$page?>');" style="color:blue;"><b><?=$i?></b></a>
					<?
					}
...
;					}
			...
			###다음버튼
				if($total_page > $end_page){
					...
					?>
						<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$next_list?>');" class="next">NEXT</a>
					<?
					// 처음
