...
if($num >= 1){ // DB 추출갯수가 하나라도 있을때 시작 
	...
	//echo "시작페이지 = ".$start_page." | 현재페이지 = ".$c_page." || current_page = ".$current_page." ||| 종료페이지 = ".$end_page." |||| 총 페이지수 = ".$total_page."<br><br>";
			...
			###처음 버튼
				if ($c_page > $page_size) {
					//마지막
					echo '<a href="javascript:paging_sub('.$total_param.');"><img src="'.$last_img_path.'" border="0"  alt="처음으로"/></a>';
					//이전
					...
					echo '<a href="javascript:paging_sub('.$prev_list.');"><img src="'.$prev_img_path.'" border="0"  alt="이전으로"  /></a>';
				}
				//echo '&nbsp;';
			###페이지 출력
				for ($i=$start_page;$i <= $end_page;$i++){
					...
					if($c_page != $page){
						...
						echo '<span><a href="javascript:paging_sub('.$page.');">'.$i.'</a></span>';
					}else{
						//echo "<span class='page'>".($i+1).'</span>';
						echo '<span class="on"><a href="javascript:paging_sub('.$page.');">'.$i.'</a></span>';
					}
...
;					}
			...
			###다음버튼
				if($total_page > $end_page){
					...
					echo '<a href="javascript:paging_sub('.$next_list.');"><img src="'.$next_img_path.'" border="0"  alt="다음으로"/></a>';
					// 처음
					...
					echo '<a href="javascript:paging_sub('.$last_page.');"><img src="'.$first_img_path.'" border="0"   alt="마지막으로" /></a>';
				}
