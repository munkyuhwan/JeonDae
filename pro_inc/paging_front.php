...
if($num >= 1){ // DB 추출갯수가 하나라도 있을때 시작 
	...
	//echo "시작페이지 = ".$start_page." | 현재페이지 = ".$c_page." || current_page = ".$current_page." ||| 종료페이지 = ".$end_page." |||| 총 페이지수 = ".$total_page."<br><br>";
			...
			###처음 버튼
				if ($c_page > $page_size) {
					//마지막
					//echo '<a href="'.$_SERVER["PHP_SELF"].'?pageNo=&'.$total_param.'" class="prev end">첫 페이지</a>';
					//이전
					...
					echo '<li><span class="prev"><a href="'.$_SERVER["PHP_SELF"].'?pageNo='.$prev_list.'&'.$total_param.'">이전</a></span></li>';
				}
				//echo '&nbsp;';
			###페이지 출력
				for ($i=$start_page;$i <= $end_page;$i++){
					...
					if($c_page != $page){
						...
						echo '<li class="active"><a href="'.$_SERVER["PHP_SELF"].'?pageNo='.$page.'&'.$total_param.'" class="on">'.$i.'</a></li>';
					}
...
;					}
			...
			###다음버튼
				if($total_page > $end_page){
					...
					echo '<li><span class="next"><a href="'.$_SERVER["PHP_SELF"].'?pageNo='.$next_list.'&'.$total_param.'">다음</a></span></li>';
					// 처음
					...
					//echo ' <a href="'.$_SERVER["PHP_SELF"].'?pageNo='.$last_page.'&'.$total_param.'" class="next end">마지막 페이지</a>';
				}
